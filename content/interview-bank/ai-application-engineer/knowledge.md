...
Tags: openai-api, model-selection, provider-adapter, streaming, retry, cost, observability
...
Tags: prompt-engineering, context-engineering, instruction-hierarchy, prompt-injection, evals
...
## 结构化输出与工具协议
Competency: 结构化输出
Tags: structured-output, json-schema, tool-schema, validation, audit, idempotency
...
## RAG 数据治理
Competency: RAG 数据治理
Tags: rag, ingestion, chunking, metadata, acl, versioning, data-quality
...
## Embedding 与向量检索
Competency: 向量检索
Tags: embedding, vector-db, hybrid-search, rerank, pgvector, recall-at-k, metadata-filter
...
## RAG 生成控制与评估
Competency: RAG 评估
Tags: rag-evaluation, groundedness, faithfulness, citation, golden-set, llm-judge
...
Tags: agent, langgraph, workflow, tool-calling, checkpoint, human-in-the-loop
...
## Agent 状态与记忆
Competency: 对话状态管理
Tags: agent-memory, session-state, message-history, checkpoint, privacy, ttl
...
## 生产化与可观测性
Competency: 生产排查
Tags: observability, trace, latency, token-usage, cost, regression-eval, rollback
...
## AI 安全与权限治理
Competency: AI 安全
Tags: llm-security, prompt-injection, data-leakage, tool-permissions, privacy, audit
...
## AI 前端与交互体验
Competency: AI 前端工程
Tags: streaming-ui, sse, websocket, ndjson, retry, idempotency, session-recovery
...
## 系统设计与产品判断
Competency: 系统设计
Tags: system-design, interview-agent, question-bank, governance, versioning, multi-tenant
