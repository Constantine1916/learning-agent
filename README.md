...
Learning Agent 是一个真实的 AI 应用开发工程师面试官 Agent。它从简历和自我介绍开始，基于 AI 应用工程师知识题库 RAG 动态提问，并在面试结束后统一生成结构化评分和最终通过结果。
...
- `knowledge.md` 和 `collected-knowledge.jsonl` 会切分、embedding 后写入 `knowledge_chunks`，作为面试官动态追问、最终评分参考和知识补充的 RAG 上下文。
...
当前确定性校准会验证优秀、合格、较差样本是否落在预期分数带；真实面试结束后仍由 LLM judge 结合 rubric、校准样本和完整问答统一完成评分。

## API

- `POST /api/resumes/upload`
- `POST /api/interviews`：传 `resumeId` 创建简历面试；传 `mode: "practice"` 创建快速练题
- `POST /api/interviews/:id/self-introduction`
- `POST /api/interviews/:id/messages`
- `GET /api/interviews/:id/report`

`POST /api/interviews/:id/messages` 使用 `application/x-ndjson` 事件流返回 Agent 运行过程。前端通过 `fetch().body.getReader()` 读取事件并增量渲染下一题：
...
其中 `message.delta` 来自 OpenAI-compatible `stream: true` 模型调用，用于实现面试官下一题的逐段流式渲染。`assessment.started` / `assessment.done` 只会在最后一轮答完后触发，用于一次性生成每题评分和最终报告。
